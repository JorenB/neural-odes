"""
Stage 2 -- Latent ODE on NOISY q observations.

Same architecture as Stage 1:
  Encoder  g_phi : (q[t0], q[t0+dt], ..., q[t0+(k-1)dt]) --> z0 in R^2
  ODE      f_theta : z --> dz/dt   (autonomous MLP)
  Decoder  h : z --> q             (FIXED: h(z) = z[0])

What changes vs Stage 1
-----------------------
- `data.obs_noise > 0`: every observation of q is q_true + epsilon, where
  epsilon ~ N(0, sigma^2). Train AND eval encoder windows are noisy.
- Eval rollout MSE is averaged over `n_eval_seeds` independent noise
  realizations to reduce variance from the single-encoder-window draw.
- Final figures are rendered from one noise realization (so you can see
  what a typical noisy roll-in produces), but the MSE numbers in the log
  are seed-averaged.

Headline question
-----------------
Does the deterministic encoder gracefully *absorb* observation noise, or
does it propagate noise into z0 and then amplify it over the rollout? The
answer sets up the case for Stage 3 (variational encoder): if noise is
absorbed gracefully here, we may not need it; if it's amplified, the
variational version's calibrated uncertainty becomes useful.

The training-loss noise floor at sigma=0.05 is ~sigma^2 = 2.5e-3, which is
~20x above Stage 1's converged loss. We expect rollout MSE to degrade as
the encoder propagates noise into its 2D latent.
"""
...
def main(cfg: DictConfig):
    ...
    def rollout_from(A_t, t_grid, noisy_encoder=True):
        """Encode initial window for each A, integrate over t_grid.
        Returns (q_pred: (T_grid, B), z: (T_grid, B, latent_dim)).

        If noisy_encoder, adds Gaussian noise (cfg.data.obs_noise) to the
        encoder input -- matches the training condition. Set False to see
        what the model produces from a CLEAN encoder window (useful for
        isolating "what the dynamics would do" from "encoder noise input")."""
    ...
    def eval_rollout_mse():
        # Average over multiple noise realizations so the metric isn't
        # dominated by the variance of a single noisy encoder draw.
        ...
        for _ in range(n_seeds):
            q_pred, _ = rollout_from(test_amps_t, t_plot, noisy_encoder=True)
            acc += ((q_pred - truth_TB) ** 2).mean(dim=0)            # (n_amps,)
        acc /= n_seeds
    ...
    fig.suptitle(f"Predicted q(t) from drop-p latent ODE  (noisy q, σ={cfg.data.obs_noise})")
    fig.tight_layout()
    ...
    fig.suptitle(f"Latent phase portrait vs true (q, p)  (noisy q, σ={cfg.data.obs_noise})")
    fig.tight_layout()
    ...
    fig.suptitle(f"z[1] vs true p  (noisy q, σ={cfg.data.obs_noise})")
    fig.tight_layout()
