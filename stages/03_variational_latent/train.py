...
Architecture
------------
  Encoder  g_phi : q_window --> (mu, log sigma^2) in R^2 x R^2
           Same MLP trunk as Stage 1/2, with TWO heads.
...
Loss = ||q_pred - q_obs||^2  (mean)  +  beta * KL(N(mu, sigma^2 I) || N(0, I))

What changes vs Stage 2
-----------------------
- Encoder outputs (mu, logvar). z0 is SAMPLED each forward pass; gradient
  flows through the sample via the reparam trick.
- KL term in the loss regularizes the approximate posterior toward N(0, I).
- At eval the point estimate uses z0 = mu (no sampling) -- comparable to
  Stage 2's deterministic rollout MSE.
- TWO new figures:
    * posterior_samples.png -- n_posterior_samples sampled q(t) rollouts
      per amplitude. Shows the predictive fan.
    * calibration.png       -- empirical std of sampled q(t) vs the actual
      |q_mean - q_true| residual, per amplitude. Are samples covering the
      truth, or collapsing too tight?

Headline question
-----------------
Does the variational encoder produce calibrated uncertainty? At sigma=0.05,
genuinely many z0 values are consistent with a given noisy 8-sample window.
A well-calibrated VAE samples a distribution wide enough to cover the true
trajectory; a posterior-collapsed one sits at the prior; an overconfident
one is narrow and frequently wrong outside its band.
"""
...
class VariationalEncoder(nn.Module):
    """MLP trunk + two heads producing (mu, logvar) of q(z|q_window).
    z0 is sampled outside via the reparameterization trick."""
    ...
    def forward(self, q_window):
        # q_window: (B, k_enc). Returns (mu, logvar), each (B, latent_dim).
        h = self.trunk(q_window)
        return self.head_mu(h), self.head_logvar(h)

    def sample(self, q_window):
        """Reparameterized sample. Returns (z0, mu, logvar)."""
        mu, logvar = self.forward(q_window)
        std = (0.5 * logvar).exp()
...
def main(cfg: DictConfig):
    ...
    # --- 2. Model + optimizer ---------------------------------------------
    encoder = VariationalEncoder(k_enc=k_enc, latent_dim=cfg.model.latent_dim,
    ...
    def encoder_input(A_t, noisy=True):
        """Build the encoder input window (B, k_enc) of q observations."""
        t_enc = (torch.arange(k_enc, dtype=torch.float32) * dt)[:, None].expand(
            k_enc, len(A_t))                                          # (k_enc, B)
        q_enc = truth.state(A_t, t_enc)[..., 0].T                    # (B, k_enc)
        if noisy:
            q_enc = q_enc + cfg.data.obs_noise * torch.randn_like(q_enc)
        return q_enc

    def rollout_from(A_t, t_grid, noisy_encoder=True, sample=False):
        """Point-estimate rollout. z0 = mu (default) or one reparam sample.
        Returns (q_pred: (T_grid, B), z: (T_grid, B, latent_dim))."""
        q_enc = encoder_input(A_t, noisy=noisy_encoder)
        mu, logvar = encoder(q_enc)
        if sample:
    ...
    def posterior_samples(A_t, t_grid, n_samples, noisy_encoder=True):
        """Return (n_samples, T_grid, B) of q(t) from posterior samples.
        Uses ONE noisy encoder window per amplitude, then n_samples reparam
        draws from that posterior."""
        q_enc = encoder_input(A_t, noisy=noisy_encoder)
        mu, logvar = encoder(q_enc)
        std = (0.5 * logvar).exp()
        samples = []
        ...
        return torch.stack(samples, dim=0)                            # (S, T, B)

    @torch.no_grad()
    def eval_rollout_mse():
        # Average over multiple NOISE realizations (each redraws the noisy
        # encoder input). Point estimate uses z0 = mu (no posterior sample).
        ...
        for _ in range(n_seeds):
            q_pred, _ = rollout_from(test_amps_t, t_plot,
                                     noisy_encoder=True, sample=False)
    ...
    for step in range(1, cfg.train.steps + 1):
        ...
        z0, mu, logvar = encoder.sample(q_obs[:k_enc].T)             # (B, latent) each
        z = odeint(func, z0, local_t, method=method)                 # (T, B, latent)
        q_pred = decode_q(z)                                         # (T, B)
        recon = torch.mean((q_pred - q_obs) ** 2)
    ...
    fig.suptitle(f"Predicted q(t) from drop-p latent ODE  (variational, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
    ...
    fig.suptitle(f"Latent phase portrait vs true (q, p)  (variational, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
    ...
    fig.suptitle(f"z[1] vs true p  (variational, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
    ...
    with torch.no_grad():
        q_samples = posterior_samples(test_amps_t, t_plot, n_samp,
                                      noisy_encoder=True)            # (S, T, B)
