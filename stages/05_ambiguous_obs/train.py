"""
Stage 5 -- Ambiguous observation map: observe o = q^2 instead of q.

What changes vs Stage 4
-----------------------
The single change is the observation function. Instead of observing q + noise,
the encoder now receives o = q^2 + noise. This is a globally two-to-one map:
for any positive o, the true q could have been +sqrt(o) or -sqrt(o). The
posterior over z0 (specifically z[0] = q at t=0) is therefore genuinely
*bimodal*: both branches are equally consistent with the data.

Everything else is identical to Stage 4: GRU encoder over sparse irregular
(t_i, o_i) pairs, variational posterior, latent ODE, fixed decoder q = z[0].
The recon loss now compares predicted o = q_pred^2 to observed o, since q
itself is no longer observable.

Headline question
-----------------
Our variational encoder outputs a DIAGONAL GAUSSIAN posterior. A diagonal
Gaussian fundamentally can't represent two well-separated modes -- it's
unimodal. So what does the model do with genuinely bimodal evidence?

Three possible behaviors:
  - Mode collapse: per window, commit to one branch (+q or -q). Reconstruct
    that branch well; the predicted std doesn't reflect the actual ambiguity.
  - Mode averaging: place mu near 0 (between branches), inflate sigma. Honest
    but useless for prediction.
  - Sample-dependent mixture: sometimes one, sometimes the other.

The posterior_samples figure will tell us which. We also report q-rollout
MSE (sign-sensitive) and o-rollout MSE (sign-agnostic) to separate
"got the orbit right" from "got the sign right."
"""
...
def main(cfg: DictConfig):
    ...
    def sample_obs_window_at(A_t, phase0):
        ...
        return them sorted ascending along with the corresponding noisy o = q^2
        observations. phase0 (B,) is the absolute time at which the window's
        local-time 0 starts -- random during training, ZERO at eval."""
        Bn = len(A_t)
        ...
        # Ambiguous observation function: o = q^2 + noise.
        o_obs = q_true ** 2 + cfg.data.obs_noise * torch.randn_like(q_true)
        return obs_local, o_obs

    def sample_batch():
        """Per-step batch: random amplitudes, random phases, random irregular
        observation windows, AND a shared dense target grid (in o-space)."""
        A_idx = torch.randint(0, len(train_amps_t), (B,))
        A = train_amps_t[A_idx]                                       # (B,)
        phase0 = torch.rand(B) * phase_max                            # (B,)
        obs_local, o_obs = sample_obs_window_at(A, phase0)            # (B, N) each
        target_abs = phase0[None, :] + local_t[:, None]               # (T, B)
        q_target_true = truth.state(A, target_abs)[..., 0]            # (T, B)
        o_target = q_target_true ** 2 + cfg.data.obs_noise * torch.randn_like(q_target_true)
    ...
    def eval_rollout_mse():
        """Returns (q_mse_dict, o_mse_dict). q-MSE is sign-sensitive (compares
        q_pred to true q). o-MSE compares q_pred^2 to true q^2 -- sign-agnostic,
        the right metric for 'did you reconstruct the observable?'"""
        ...
        for _ in range(n_seeds):
            ...
            acc_q += ((q_pred - truth_TB) ** 2).mean(dim=0)
            acc_o += ((q_pred ** 2 - truth_TB ** 2) ** 2).mean(dim=0)
    ...
    for step in range(1, cfg.train.steps + 1):
        optimizer.zero_grad()
        obs_local, o_obs, o_target = sample_batch()                  # (B, N), (B, N), (T, B)
        z0, mu, logvar = encoder.sample(obs_local, o_obs)            # (B, latent) each
        z = odeint(func, z0, local_t, method=method)                 # (T, B, latent)
        q_pred = decode_q(z)                                         # (T, B)
        o_pred = q_pred ** 2                                         # (T, B)
        recon = torch.mean((o_pred - o_target) ** 2)
        ...
        if step % cfg.train.log_every == 0 or step == 1:
            ...
            # Best model selected by o-MSE (the sign-agnostic / observable metric).
            seen_err = sum(o_errs[A] for A in train_amps) / len(train_amps)
    ...
    fig.suptitle(f"Predicted q(t) from drop-p latent ODE  (ambiguous obs o=q², GRU, N={n_obs} over {horizon}s, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
    ...
    fig.suptitle(f"Latent phase portrait vs true (q, p)  (ambiguous obs o=q², GRU, N={n_obs} over {horizon}s, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
    ...
    fig.suptitle(f"z[1] vs true p  (ambiguous obs o=q², GRU, N={n_obs} over {horizon}s, σ={cfg.data.obs_noise}, β={beta_kl})")
    fig.tight_layout()
